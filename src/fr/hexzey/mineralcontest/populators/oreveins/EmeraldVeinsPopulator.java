...
public class EmeraldVeinsPopulator extends BlockPopulator
{
	@Override
	public void populate(World world, Random random, Chunk chunk)
	{
		int X, Y, Z;
		for(int i = 1; i < 3; i++) // nombre max de filons par chunk
		{
		    if(random.nextInt(100) < 50) // probabilité de générer un filon
		    {
				...
				Y = random.nextInt(8)+1+1; // hauteur aléatoire
				...
				{
					int longueurFilon = 1;
					GenererFilon.genererFilon(new Location(world, X, Y, Z), Material.EMERALD_ORE, longueurFilon);
				}
		    }
		}
	}
